...
function AdminUsers () {
    ...
    const [userResultater, setUserResultater] = useState("Søg i gruppespil");
    ...
    useEffect(() => {
        ...
        axios.get(URL, requestConfig).then(result => {
            setItems(result.data.allUsers);
            setUserCount(result.data.allUsers.length);
            setUserResultater("Søg i " + result.data.allUsers.length + " resultater");
            ...
            const labels = [
              ...
              for (var i in result.data.allUsers) {
                const timeO = new Date(result.data.allUsers[i].oprettelse).getTime();
                ...
                <div className="admin-section">
                    <div className="admin-section-big">
                        <div>
                            ...
                            {loading}
                            <canvas id="userChart" className="admin-gruppespil-main-canvas" width="600px" height="150px"></canvas>
                        </div>
                    </div>
                ...
                <div className="admin-section">
                    <div className="admin-section-large admin-section-large-gruppespil">
                        <div className="admin-section-table-top">
                            <p className="admin-canvas-label" style={{padding: "0px"}}>Brugere</p>
                        ...
                        <div className="admin-table-filters">
                            <div className="gruppespil-input-div">
                                ...
                                <input type="text" className="login-form-input" style={{paddingLeft: "45px"}} placeholder={userResultater}/>
                            </div>
                        ...
                        <div className="admin-table">
                            <div className="admin-table-top">
                                ...
                                <p className="admin-table-label admin-navn">Fulde navn</p>
                                <p className="admin-table-label admin-spillere">Brugernavn</p>
                                <p className="admin-table-label admin-oprettelse">Oprettelse</p>
                                <p className="admin-table-label admin-admin">Email</p>
                                <p className="admin-table-label admin-synlighed">Rolle</p>
                            </div>
                            <div className="admin-table-section">
                                {loading}
                                <ul>
                                    {items.map((item) => {
                                        const userURL = "/admin/profil?id=" + item.id;
                                        return (
                                            <li key={item.id}>
                                                <Link to={userURL}>
                                                <div className="admin-table-cell">
                                                    <div className="admin-table-check"></div>
                                                    <p className="admin-table-label admin-navn">{item.fornavn} {item.efternavn}</p>
                                                    <p className="admin-table-p admin-spillere">{item.username}</p>
                                                    <p className="admin-table-p admin-oprettelse">{item.oprettelse}</p>
                                                    <p className="admin-table-p admin-admin">{item.email}</p>
                                                    <p className="admin-table-p admin-synlighed">{item.rolle}</p>
                                                </div>
                                                </Link>
                                            </li>
                                            );
